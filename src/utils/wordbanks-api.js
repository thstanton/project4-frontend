...
function newWordbank(data) {
    const response = axios.post(`${API_URL}/contexts/wordbank/create/`, data, { headers: headers() })
    return response
}
...
function newWordList(data) {
    const response = axios.post(`${API_URL}/contexts/word/createlist/`, data, { headers: headers() })
    return response
}
